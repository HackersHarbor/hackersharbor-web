...
import { SQLExecutionService } from "../services/execution/SQLExecutionService";
...
export function CodeMessage({ code }: CodeMessageProps) {
  ...
  const handleRun = async () => {
    ...
    try {
      const normalizedLanguage = code.language
        .trim()
        .toLowerCase();
      ...
      if (
        ...
        normalizedLanguage === "duckdb"
      ) {
        const result = await sqlExecutionService.execute(editorCode);
        ...
        setLogs((current) => [
          ...current,
          `[${finishedAt}] DuckDB-WASM execution completed`,
          `[${finishedAt}] Rows returned: ${result.rowCount}`,
